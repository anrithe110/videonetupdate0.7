...
function AddBanner() {
    ...
    function Imgs({ file, index, onRemove }) {
        return (
            <div className="!p-[7px] imgs max-h-14" key={index}>
                <img className='!rounded' src={URL.createObjectURL(file)} alt={`Image Preview ${index}`} />
                <button
                    className="absolute bottom-[-4rem] right-[1.4rem]"
        ...
        <>
            ...
            <div className="inputpart">
                ...
                <div className="flex flex-col gap-3 pt-4 items-center">
                    <button className="btn" onClick={AddBanner} disabled={!name || imageFiles.length !== 1}>
                        Add Banner
                    </button>
                </div>
            </div>
