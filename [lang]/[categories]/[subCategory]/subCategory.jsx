...
export default function CategoriesPage() {
  ...
  const { categories, subCategory } = useParams();
      ...
      if (categories !== undefined && categories !== null) {
        try {
          const response = await fetch(
            `${url}/api/ParentCategory?url=${categories}`,
            {
              method: "GET",
              headers: {
          ...
          if (response.ok) {
            ...
            setBrands(data?.category?.brands);
    ...
    const fetchCategoriesAndProducts = async () => {
      try {
        ...
        if (categoryResponse.ok) {
          ...
          const stats = findStatByURL(data.category, subCategory);
          setCategory(stats);
          if (stats) {
            const productResponse = await fetch(
              `${url}/api/Products/findByCategory?id=${stats._id}`,
              {
                method: "GET",
                headers: { "Content-Type": "application/json" },
              }
  ...
  const filteredProducts = products
    ...
    .map((product) => {
      ...
      const matchedStats = product.stats.filter((stat) => {
        return category.stats.some((c) => {
          const query = searchParams.get(c.id);
          if (query !== null) {
            const filters = query.split(",");
            console.log(stat.id === c.id);
            return stat.id === c.id && filters.includes(stat.value);
          }
          return false;
        });
      });

      return matchedStats.length > 0
        ? { ...product, stats: matchedStats }
        : null;
    })
  ...
  return (
    <NavTer classNameBottom="!pb-[55px]">
      <div className="w-full h-fit items-center">
        ...
        <Linked link={`/${ParentCategory?.url}`} name={ParentCategory && ParentCategory.name[localActive]} />
          <Linked link={`/${ParentCategory?.url}/${category?.url}`} name={category && category.name[localActive]} />
